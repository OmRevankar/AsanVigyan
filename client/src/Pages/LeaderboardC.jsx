      ...
      <div className="max-w-4xl mx-auto px-4 mt-12">
        ...
        <div className="grid grid-cols-3 gap-4 items-end mb-12 px-2">
          {topThree.map((item, index) => {
            ...
            return (
              <div key={item._id} className={`flex flex-col items-center ${order}`}>
                <div className="relative mb-3">
                  ...
                  <div className={`relative size-20 md:size-24 rounded-full bg-white border-4 ${rankAssets[index].border} ${rankAssets[index].shadow} shadow-xl p-1 transition-transform hover:scale-110 cursor-pointer`}
                    onClick={() => handleUserClick(item._id)}>
                    <img src={rankAssets[index].img} alt={`Rank ${index + 1}`} className="w-full h-full object-contain" />
                  </div>
                </div>

                {/* Podium Highlight Logic */}
                <div className={`w-full ${height} ${isCurrentUser ? 'bg-purple-50 border-purple-200 ring-2 ring-purple-500/20' : 'bg-white border-slate-100'} rounded-t-3xl shadow-sm border flex flex-col items-center justify-center p-2 text-center relative overflow-hidden`}>
                  ...
                  <p className="text-purple-600 font-black text-xl md:text-2xl relative z-10">{item.attempts}</p>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest relative z-10">Total Tests</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="grid grid-cols-12 px-6 py-4 border-b border-slate-50 text-xs font-bold text-slate-400 uppercase tracking-widest">
            ...
            <div className="col-span-3 text-right flex items-center justify-end gap-1">
               Attempts <Zap size={12} className="fill-purple-600 text-purple-600"/>
            </div>
          </div>
                  ...
                  <div className="col-span-7 flex items-center gap-3">
                    <div>
                      ...
                      <p className={`font-bold text-sm ${isCurrentUser ? 'text-purple-700' : 'text-slate-700'}`}>
                        {item.username} {isCurrentUser && <span className="ml-2 text-[10px] bg-purple-600 text-white px-1.5 py-0.5 rounded-full uppercase font-bold">You</span>}
                      </p>
                      <p className="text-xs text-slate-400 font-medium">UID: {item.uid || 'N/A'}</p>
                    </div>
                  </div>

                  <div className="col-span-3 text-right">
                    <span className="font-black text-slate-800 tracking-tight text-lg">{item.attempts}</span>
                  </div>
                </div>
              );
            })}
        ...
        {leaderboard.length === 0 && (
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 font-medium tracking-tight">The arena is empty. Start a test to claim your spot!</p>
          </div>
        )}
      </div>
    </div>
